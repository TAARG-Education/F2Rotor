...
def BladeSection_alpha_Mach(Omega, R, lambda_, r_segn, beta, dbeta, mu, psi, theta, alpha_stall_up, alpha_stall_lo, h):
    ...
    # Initialize array for non-stall region
    ...
    non_stall_condition = (alpha_e < np.radians(alpha_stall_up)) & (alpha_e > np.radians(alpha_stall_lo))
    non_stall[non_stall_condition] = alpha_e[non_stall_condition]
    M = V_eff / a_inf
