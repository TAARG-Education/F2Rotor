...
#x Simple estimation of drag coefficient f_Af and equivalent wetted area f_f of the fuselage in forward flight in case the following geometrical data and basic parameters are known:        x
...
#x                                                                                                                                                                                           x
#x Procedure:                                                                                                                                                                                x
#x  1. For compound helicopters: calculation of wing-fuselage interference factor R_WB from experimental data stored in the values.json file; for conventional helicopters: R_WB = 1 is      x
#x     assumed (inter_fact function)                                                                                                                                                         x
...
#x                                                                                                                                                                                           x
#x References:                                                                                                                                                                               x
#x  - Carlo de Nicola, (2018-2019), Appunti per un corso di Aerodinamica degli Aeromobili, Appendix E: http://wpage.unina.it/denicola/AdA/DOWNLOAD/Appunti_AdA_2018_2019.pdf                 x                                                                    x
...
#x                                                                                                                                                                                           x
#x Notes: for usage examples refer to test_cases_parasite_fus.py                                                                                                                             x                                                                                       x
#x                                                                                                                                                                                           x
#x Author: Iole Paolillo                                                                                                                                                                     x
#x Latest update: 19/07/2024                                                                                                                                                                 x
#x Version: 1.0                                                                                                                                                                              x                                                                                                                                                        x                                                                                                            x                                                                                                                               x
#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
class DataImport:                                
    ...
    def inter_fact(self):
        '''Function that evaluates the fuselage and wing correlation factor R_WB. The correlation factor is interpolated from experimental curves (data are present within the values.json file)
	       - Input: experimental data contained within the values.json file
	       - Output: R_WB (fuselage-wing correlation factor)
        '''
        if self.helicopter_type == "conventional":
           self.R_WB = 1
        else:
            ...
            if self.Re_inf < max_min:
               print('Error! Reynolds number too low.')
               return None  # Return None in case of error

            if self.Re_inf > min_max:
    ...
    def fuselage(self):
        ...
        h = self.config["ParasiteArea"][0]["h"]
        d = self.config["ParasiteArea"][0]["d"]
        L = self.config["ParasiteArea"][0]["L"]
        S = self.S
        M_oo = self.M_inf
        Re_oo = self.Re_inf
