...
R = 0.06                                                                                             # Rotor radius [m].
theta0 = np.deg2rad(20.0)                                                                      # Root pitch angle [rad].
theta_end = np.deg2rad(11.9)                                                                    # Tip pitch angle [rad].
...
# Alpha_e 3D.
Vinf_mesh, r_bar_mesh = np.meshgrid(Vinf_vec, r_segn)
alpha_e_values = alpha_e_vec[:, :, 0].T
fig = plt.figure(12)
ax = fig.add_subplot(111, projection='3d')

surf = ax.plot_surface(Vinf_mesh, r_bar_mesh, alpha_e_values, cmap='viridis', edgecolor='k', alpha=0.9)

ax.set_xlabel(r'$V_\infty \ [m/s]$', fontsize=12)
ax.set_ylabel(r'$\bar{r}$', fontsize=12)
ax.set_zlabel(r'$\alpha_e \ [rad]$', fontsize=12)

ax.grid(which='major', linestyle='-', linewidth=0.75)
ax.grid(which='minor', linestyle='--', linewidth=0.5)

fig.colorbar(surf, shrink=0.5, aspect=10, label=r'$\alpha_e \ [rad]$')
